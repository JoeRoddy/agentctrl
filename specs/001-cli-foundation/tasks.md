...
- [X] T002 Initialize package.json with name "agentctl", type "module", bin field pointing to `./dist/cli.js`
...
**Goal**: A developer runs the agentctl CLI and sees a hello world response

**Independent Test**: Run `node dist/cli.js` and verify "Hello from agentctl!" appears
...
- [X] T014 Run quickstart.md validation (npm link and test `agentctl` command)
...
| SC-002: Running agentctl displays output | T011 | "Hello from agentctl!" appears |
