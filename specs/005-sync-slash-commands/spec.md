...
- Q: For Codex only, when converting slash commands to skills, should the user choose scope? → A: Prompt the user to choose global (default), project (converted to a skill), or skip.
...
**Independent Test**: Can be fully tested by defining a small set of commands and running a sync to two supported agents, then confirming the commands appear in each agent at the chosen scope.

**Acceptance Scenarios**:

1. **Given** a set of custom slash commands and two agents that support custom commands, **When** I run sync with a chosen scope per agent, **Then** the commands are available in each agent at that scope and the summary reports the actions taken.
...
### User Story 2 - Choose fallbacks for unsupported agents (Priority: P2)

As a user, when an agent does not support custom slash commands, I want to choose a fallback so I can still get value from my commands or intentionally skip that agent.

**Why this priority**: Users need a graceful path for agents without support to avoid confusion and lost work.

**Independent Test**: Can be fully tested by selecting an unsupported agent and choosing each fallback option, then verifying the reported outcome.

**Acceptance Scenarios**:

1. **Given** an agent that does not support custom slash commands, **When** I opt to convert commands into skills, **Then** those skills are created and listed with the original command names.
2. **Given** an agent that does not support custom slash commands, **When** I opt to skip, **Then** no commands are applied for that agent and the summary reports it as skipped.
...
1. **Given** Codex is selected as a sync target, **When** sync starts, **Then** I am warned about the lack of project-level custom commands and offered options to configure globally, convert to skills, or skip.
...
3. **Given** I choose to convert to skills for Codex, **When** I am prompted for scope, **Then** global is the default option and I can choose global, project, or skip with the warning that project scope may not work in Codex.
...
- **FR-003**: The system MUST let users choose the command scope per agent when an agent supports more than one scope.
...
- **FR-007**: For agents that do not support custom slash commands, the system MUST offer an opt-in conversion that maps commands into skills or allow the user to skip that agent.
- **FR-008**: For Codex, the system MUST warn that project-level custom slash commands are not supported and provide options to configure globally, convert to skills, or skip.
...
- **FR-012**: When Codex conversion to skills is selected, the system MUST prompt the user to choose global (default), project, or skip, and surface that project scope may not function in Codex.
...
- **Sync Decision**: The per-agent choices made during a sync (scope, convert to skill, or skip).
- **Skill Mapping**: The representation of a slash command converted into a skill, retaining the original command name and the chosen scope (project or global) when applicable.
...
- When an agent supports both project and personal/global scopes, the default choice is project scope unless the user selects otherwise.
- Skill conversions create project-scoped skills by default; for Codex, users are prompted to choose global (default) or project scope, or skip conversion.
...
- **SC-003**: At least 90% of users successfully complete a fallback choice (convert to skills or skip) for unsupported agents on the first attempt.
